...
def render_peoc_case_study_markdown(payload: JsonDict, *, language: str) -> str:
    ...
    lines.extend(
        [
            ...
            (
                f"- {labels['held_rho']}: "
                f"{_md(stage.get('held_spearman_rho', 'unknown'))}"
            ),
            (
                f"- {labels['held_ci']}: "
                f"{_md(_format_value(stage.get('held_bootstrap_ci')))}"
            ),
            "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                f"| {labels['role']} | {labels['relative_path']} | "
                f"SHA-256 | {labels['bytes']} |"
            ),
            "|---|---|---|---:|",
        ]
    )
    ...
    lines.extend(
        f"- {_md(value)}" for value in limitations
    )
    lines.extend(
        [
            "",
            ...
            "",
            (
                f"- {labels['full_support']}: "
                f"`{_md(boundary.get('full_research_support', False))}`"
            ),
