...
def _summary(
    ...
    required = ", ".join(
        f"{name}={check.get('status')}" for name, check in required_checks.items()
    )
    advisory = ", ".join(
        f"{name}={check.get('status')}" for name, check in advisory_checks.items()
    )
...
def _render_markdown(payload: JsonDict) -> str:
    ...
    actions = "\n".join(
        f"{index}. {item}"
        for index, item in enumerate(_list(payload.get("next_actions")), 1)
    )
    return "\n".join(
        [
