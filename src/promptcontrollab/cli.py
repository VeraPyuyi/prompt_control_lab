...
def _cmd_choose(args: argparse.Namespace) -> None:
    ...
    if args.need is None:
        payload = {"choices": _tool_choice_lanes(), "next": "Run pcl choose --need <your-goal>."}
    else:
        payload = _choose_tool_for_need(args.need)
    if args.json:
        print(json.dumps(payload, indent=2, ensure_ascii=False, sort_keys=True))
        return
    print(_format_tool_choice(payload, language=args.language))


def _tool_choice_lanes() -> list[JsonDict]:
    return [
        {
            "id": "security",
            "use_first": "Promptfoo",
        ...
        },
        {
            ...
            "pcl_adds": (
                ...
                "soft-hard gap, "
                "trajectory, Riccati, and time-varying soft-control diagnostics."
            ),
            "commands": [
...
def _choose_tool_for_need(need: str) -> JsonDict:
    text = need.lower().strip()
    lanes = _tool_choice_lanes()
...
def _format_tool_choice(payload: JsonDict, *, language: str = "en") -> str:
