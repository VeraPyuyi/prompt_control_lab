...
from promptcontrollab.evidence.posttrain_pilot import aggregate_pilot_decisions
...
def write_pilot_summary(root: Path, *, seeds: tuple[int, ...]) -> JsonDict:
    ...
    checkpoint_metrics: dict[str, list[JsonDict]] = {stage: [] for stage in ("initial", *stages)}
    ...
    delta_summary = {stage: _numeric_summary(values) for stage, values in stage_deltas.items()}
    checkpoint_summary = {
...
def _checkpoint_measurements(checkpoint_dir: Path) -> JsonDict | None:
    if (
        not (checkpoint_dir / "manifest.json").is_file()
        or not (checkpoint_dir / "metrics.json").is_file()
    ):
        return None
    ...
    return {
        ...
        "generation_saturation_rate": _optional_float(metrics.get("generation_saturation_rate")),
