...
def _validate_queue_snapshot(
    ...
    *,
    ...
    """Validate the approved queue snapshot before any pilot execution."""
    ...
    validation.update(
        {
            ...
            "external_scheduler_policy_sha256": snapshot["external_scheduler_policy_sha256"],
        }
    )
    return validation
    ...
    *,
    ...
    for index, snapshot in enumerate(snapshots, 1):
        ...
        if float(memory) > max_memory_mib:
            raise ValueError(f"GPU {gpu} has {memory} MiB allocated; refusing to start")
    if uuids[0] != uuids[1]:
...
def _model_file_records(model_path: Path) -> list[JsonDict]:
    ...
    for path in sorted(model_path.rglob("*"), key=lambda item: item.as_posix()):
        ...
        if not path.is_file() or path.name == _MODEL_PROVENANCE_FILE or ".cache" in relative.parts:
            continue
        records.append(
            {
