...
def _execute_sft_pilot_locked(
    ...
    *,
    ...
    """Execute the SFT pilot while holding validated GPU and output locks."""
    ...
    *,
    ...
    for name, raw_path in resources.items():
        ...
        if expanded.is_symlink():
            raise PosttrainPilotError(f"Pilot {name} path must not be a symbolic link: {expanded}")
...
def _run_pilot(inputs: PilotInputs, *, model_provenance: JsonDict) -> None:
    """Train and evaluate all configured checkpoints for one controlled pilot."""
...
def _evaluate_checkpoint(
    *,
    ...
    """Evaluate one checkpoint and persist bounded metrics and diagnostics."""
    ...
    representation_centroid = [value / len(rows) for value in (representation_sum or [])]
