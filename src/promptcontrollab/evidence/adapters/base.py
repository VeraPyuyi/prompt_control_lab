...
class GenericMetricAdapter:
    ...
    def build(self, rows: list[JsonDict]) -> JsonDict:
        ...
        metrics = {name: _numeric_summary(items) for name, items in sorted(values.items()) if items}
        if metrics:
            observation = (
...
def _source_reference(row: JsonDict) -> JsonDict:
    ...
    return {
        ...
        "source_path_sha256": ("sha256:" + hashlib.sha256(relative.encode("utf-8")).hexdigest()),
