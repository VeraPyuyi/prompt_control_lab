...
class _SourceSnapshot:
    ...
    def read(self, path: Path) -> JsonDict:
        """Read and verify one JSON artifact beneath the captured source root."""
...
def _checkpoint_rows(
    ...
    *,
    ...
    """Collect portable checkpoint measurements for the selected pilot seeds."""
...
def _gate_rows(
    ...
    *,
    ...
    """Collect and validate checkpoint-gate decisions for export."""

    rows: list[JsonDict] = []
    checkpoint_index = {(int(row["seed"]), str(row["stage"])): row for row in checkpoint_rows}
    for seed in seeds:
        for stage in _GATE_STAGES:
            ...
            if decision == "hold" and not any(item["impact"] == "hold" for item in triggered):
...
def _portable_provenance(
    ...
    *,
    ...
    """Build public provenance metadata without copying private training payloads."""
...
def _public_summary(
    ...
    *,
    ...
    """Create the bounded public pilot summary from checkpoint and gate rows."""
    ...
    for stage in _STAGES:
        ...
        metrics_by_stage[stage] = {
            key: _aggregate_numbers([float(row[key]) for row in stage_rows]) for key in metric_keys
        }
...
def _render_report(
    ...
    """Render the portable checkpoint pilot summary as Markdown."""
    ...
    for row in gate_rows:
        names = ", ".join(
            str(check.get("check")) for check in row["triggered_checks"] if isinstance(check, dict)
        )
        lines.append(
