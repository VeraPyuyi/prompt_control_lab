...
Paper-derived prompt diagnostics plus local evidence for evals, provenance, and agent audits. Package: `promptcontrollab`. Chinese: [README.zh.md](README.zh.md).
...
- **Paper research core:** one-click `pcl research-demo` -> `pcl diagnose`; deeper `soft-hard`, `trajectory`, `riccati`, `tv-soft`.
- **Evidence bridge:** import external evals with `pcl import promptfoo --input results.json --out runs/from-promptfoo --prompt-id candidate`; `pcl ingest` remains the backward-compatible alias for `pcl import`.
- **Applied Agent Layer:** `pcl guard`, `audit-diff`, model provenance, UI, plugins, and GitHub templates.
