...
`prompt_control_lab` turns prompt optimization experiments into auditable evidence: clean splits, paired statistics, soft-to-hard gaps, hidden-state trajectory diagnostics, Riccati probes, and time-varying soft-control comparisons. Package: `promptcontrollab`. Chinese docs: [README.zh.md](README.zh.md).
...
Local UI: `pcl ui --runs runs/ --policy examples/guard.policy.yaml --port 8501`. 4K walkthrough: [English MP4](docs/assets/demo/prompt_control_lab_demo.en.mp4) | [Chinese MP4](docs/assets/demo/prompt_control_lab_demo.zh.mp4).
