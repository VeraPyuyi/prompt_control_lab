# prompt_control_lab 中文实操演示视频分镜

工作标题：prompt_control_lab：AI 编码 Agent 的执行前门禁、模型溯源和可复现评测

目标时长：10-12 分钟。声音风格：更自然的中文技术教程讲解，短句、轻停顿、少念稿感。画面风格：4K 实操型 UI 操作回放，使用放大的当前界面截图、鼠标移动、点击高亮、命令卡片和结果提示。
...
先看这张工作台。prompt_control_lab 放在 AI 编码 agent 的前后两侧。agent 动手前，先做预检：指令清不清楚，范围会不会太大，有没有触碰团队 policy，token 成本是否合理。agent 改完后，再留下证据：模型身份、评测结果、gate 状态、diff 审计和历史记录。它不是安全证明，而是把感觉变成可复核的记录。
