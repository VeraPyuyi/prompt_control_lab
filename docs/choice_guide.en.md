...
| "I need Claude Code, Cursor, or Codex prompt guard integration." | `pcl install-plugin all`, then `pcl doctor --json` |
...
| You need prompt guard adapters for Claude Code, Cursor, Codex, or CI. | PCL adapters | Local policy guard before prompts reach the coding agent, plus stable JSON for hooks. |
...
| IDE or CI adapter workflow | Agent prompts are not yet guarded by team policy before execution. | `pcl install-plugin all` | `pcl doctor --json`, then adapter docs |
