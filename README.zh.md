...
论文优先的 prompt-control 工具：先生成研究证据包，再把 agent guard/audit 放在应用层。包名：`promptcontrollab`。英文版：[README.md](README.md)。

```bash
pip install -e ".[research,ui]"
pcl start --guide --language zh  # 或: pcl choose --need "安全评测" --language zh; pcl start --choice demo --language zh --out demo
