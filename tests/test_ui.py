...
def test_ui_ecosystem_choice_rows_explain_adjacent_tool_paths() -> None:
    ...
    assert len(en_rows) == 8
    assert len(zh_rows) == 8
    ...
    assert "Claude Code hooks" in combined_en
    assert "prompt guard adapters" in combined_en
    assert "model drift" in combined_en
    assert "Claude Code" in combined_zh
